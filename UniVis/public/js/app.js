...
module.exports = __webpack_require__.p + "neo4j-web.min.js";
...
var api = __webpack_require__(/*! ./neo4jApi */ "./resources/js/neo4jApi.js");
...
function search() {
  ...
  api.searchEmployee(query).then(function (employee) {
    var t = $("table#results tbody").empty();

    if (employee) {
      employee.forEach(function (employee) {
        $("<tr><td class='employee'>" + employee.name + "</td><td>" + employee.Project + "</td><td>" + employee.Department + "</td><td>" + employee.University + "</td></tr>").appendTo(t).click(function () {
...
function renderGraph() {
  ...
  api.getGraph().then(function (graph) {
    force.nodes(graph.nodes).links(graph.links).start();
    var link = svg.selectAll(".link").data(graph.links).enter().append("line").attr("class", "link");
    var node = svg.selectAll(".node").data(graph.nodes).enter().append("circle").attr("class", function (d) {
      return "node " + d.label;
    }).attr("r", 10).call(force.drag); // html title attribute
...
var Employee = __webpack_require__(/*! ./Employee.js */ "./resources/js/Employee.js");

var EmployeeProj = __webpack_require__(/*! ./EmployeeProj.js */ "./resources/js/EmployeeProj.js");
...
function searchEmployee(queryString) {
  ...
  return session.run('MATCH (e:EMPLOYEE) \
      WHERE e.name =~ $name \
      RETURN e', {
    name: '(?i).*' + queryString + '.*'
...
function getEmployee(name) {
  var session = driver.session();
  return session.run("MATCH (e:EMPLOYEE {name:$name})<-[:INCHARGE_OF]-(p:PROJECT) \
...
function getGraph() {
  var session = driver.session();
  return session.run('MATCH (e:EMPLOYEE)<-[:INCHARGE_OF]-(p:PROJECT) \
    RETURN e.name AS Name, p.title AS title \
    LIMIT $limit', {
    limit: 100
    ...
    var nodes = [],
        rels = [],
        i = 0;
    results.records.forEach(function (res) {
      nodes.push({
        name: res.get('Name'),
        label: 'Name'
      });
      var target = i;
      i++;
      res.get('cast').forEach(function (name) {
        var actor = {
          name: name,
          label: 'actor'
        };
...
__webpack_require__(/*! F:\xampp\htdocs\Git Clone\FYP_1_University_Hierarichal_Project\UniVis\resources\js\app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! F:\xampp\htdocs\Git Clone\FYP_1_University_Hierarichal_Project\UniVis\resources\sass\app.scss */"./resources/sass/app.scss");
